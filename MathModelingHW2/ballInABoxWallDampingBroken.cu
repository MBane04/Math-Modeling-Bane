...
void getForces()
{
	...
	//give a coefficient of friction
	float frictionCoefficient = 0.5;

	//In order to use the friction to slow the ball down we would decrease the force by the friction force.

	//Friction force = friction coefficient * normal force = u * N, N = m*g

	float normalForce = SphereMass*9.8;
	...
	if((Position.x - ballRadius) < -halfSide)
	{
		...
		printf("\n howMuch.x = %f", howMuch);
		Force.x += wallStiffness*howMuch - frictionForce*Velocity.x; //I subtracted the friction force * velocity because the friction force is always in the opposite direction of the velocity.
	}
	else if(halfSide < (Position.x + ballRadius))
	{
		howMuch = (Position.x + ballRadius) - halfSide;
		printf("\n howMuch.x = %f", howMuch);
		Force.x -= wallStiffness*howMuch+ frictionForce*Velocity.x; // so no matter what the friction force will always be in the opposite direction of the velocity resulting in an inelastic collision.
	}
	...
	if((Position.y - ballRadius) < -halfSide)
	{
		...
		printf("\n howMuch.y = %f", howMuch);
		Force.y += wallStiffness*howMuch - frictionForce*Velocity.y;
	}
	...
	if((Position.z - ballRadius) < -halfSide)
	{
		...
		printf("\n howMuch.z = %f", howMuch);
		Force.z += wallStiffness*howMuch - frictionForce*Velocity.z;
	}
...
{	
	...
	drawPicture();
	printf("\n Time = %f", RunTime);
	printf("\n Force.x = %f, Force.y = %f, Force.z = %f", Force.x, Force.y, Force.z);
	printf("\n Velocity.x = %f, Velocity.y = %f, Velocity.z = %f", Velocity.x, Velocity.y, Velocity.z);
	RunTime += Dt;
