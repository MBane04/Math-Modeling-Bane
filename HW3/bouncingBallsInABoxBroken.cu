...
void getForces()
{
	float wallStiffnessIn = 1000000.0;
	float wallStiffnessOut = 800000.0;
	float k;
	float halfSide = BoxSideLength/2.0;
	float howMuch;
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = 0; j < i; j++)
		{
			...
			float getOutOfMeStrength = 1000.0;
			if(r < (SphereDiameter+0.1))
			{
				Force[i].x += getOutOfMeStrength*(SphereDiameter - r)*dx/r;
				Force[i].y += getOutOfMeStrength*(SphereDiameter - r)*dy/r;
				Force[i].z += getOutOfMeStrength*(SphereDiameter - r)*dz/r;
				
				Force[j].x -= getOutOfMeStrength*(SphereDiameter - r)*dx/r;
				Force[j].y -= getOutOfMeStrength*(SphereDiameter - r)*dy/r;
				Force[j].z -= getOutOfMeStrength*(SphereDiameter - r)*dz/r;
			}
		}
