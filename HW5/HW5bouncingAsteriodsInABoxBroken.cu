...
// our units are 940 km, 9.383*10^20kg, 3640/3600 hours  = 1.011111 hours
...
void setInitailConditions()
{
	...
	SphereDiameter = 1.0; //diamter is 1 of our units, so in real units its 940 km
	sphereRadius = SphereDiameter/2.0;
	SphereMass = 1.0; //mass is 1 of our units, so in real units its 9.383E20 kg
	BoxSideLength = 10.0;
	MaxVelocity = 10.0;
	...
	printf("\n Max velocity = %f kilometers/hour", MaxVelocity*LengthUnitConverter/TimeUnitConverter); //Look at this later
	...
	TotalRunTime = 2400 * TimeUnitConverter; // 10 days in our units
	RunTime = 0.0;
	Dt = 0.001;
}
...
void getForces()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = 0; j < i; j++)
		{
			...
			{
				Force[i].x += GravityConstant*SphereMass*SphereMass*dx/(d*d*d); // G*m1*m2/(d^2) *rx/r is why it 
				Force[i].y += GravityConstant*SphereMass*SphereMass*dy/(d*d*d);
				Force[i].z += GravityConstant*SphereMass*SphereMass*dz/(d*d*d); 

				Force[j].x -= GravityConstant*SphereMass*SphereMass*dx/(d*d*d);
				Force[j].y -= GravityConstant*SphereMass*SphereMass*dy/(d*d*d);
				Force[j].z -= GravityConstant*SphereMass*SphereMass*dz/(d*d*d);

				//printf("\n Force = %f", Force[i].x);
			}
			
			// ??????????????????????????????????????????????????????? Solved
			// Add gravity between asteroids here. (I liked the else stament so i used that)
...
{	
	...
	printf("\n Time = %f hours = %f", RunTime, RunTime*TimeUnitConverter);
	RunTime += Dt;
