...
void setInitailConditions()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		while(test == 0)
		{
			...
			Position[i].z = maxSphereSize*sin(angle2)*sin(angle1);
	...
	{	
		...
		*/
		...
		for(int j = 0; j < i; j++)
		{
			...
			float inOut; //im sorry what is this supposed to be??????
			float kSphereReduction = 100; //how do I use these too???????????????????????
			float dvx = Velocity[j].x - Velocity[i].x;
			float dvy = Velocity[j].y - Velocity[i].y;
			float dvz = Velocity[j].z - Velocity[i].z;
